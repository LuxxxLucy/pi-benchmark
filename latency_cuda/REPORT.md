...
1. **Rerun on 3090 to capture the expanded model set with full metrics.** `src/models.py` now has 9 additional candidates (added 2026-04-17): `bert-L4-H512`, `albert-base-v2`, `electra-small`, `mobilebert`, `bge-micro-v2`, `bge-small-en-v1.5`, `modernbert-base`, `ettin-encoder-150m`, `gte-base-en-v1.5`. All smoke-test green on Mac CPU. Four of them (`modernbert-base`, `ettin-encoder-150m`, `gte-base-en-v1.5` at 8192, `bge-small-en-v1.5` would work if its max_pos reflected the RoPE extension — it doesn't) also unlock the previously-missing 1024 / 2048 / 4096 length buckets in the default sweep. `gte-base-en-v1.5` requires `trust_remote_code=True` (now threaded through `ModelSpec.trust_remote_code`). Command on the 3090: `git pull && bash run.sh`. Expect ~2 h end-to-end (24 models × up to 8 lengths × 60 iters × 2 devices; new models mostly in the <50M / 140M tiers).
...
3. **Verify testsavantai-base backbone family.** The card claims DeBERTa-v3-base (86M) but measured params are 67.0M and CUDA/CPU latency matches DistilBERT-67M exactly, not DeBERTa-v3-xsmall-70.8M. Either the card is wrong, or the architecture pulled by `AutoModelForSequenceClassification` from the checkpoint differs from the card. Check `config.json` of the checkpoint to confirm.
4. **`nomic-ai/nomic-embed-text-v1.5` integration deferred.** Its custom modeling.py (a) crashes with `TypeError: '<' not supported between instances of 'NoneType' and 'int'` unless `classifier_dropout` is explicitly passed, and (b) uses `strict=True` on `load_state_dict` so a random-head arch-baseline load fails on the missing `bert.pooler.dense.*` and `classifier.*` weights. Adding it would need a per-model loader branch; not worth it while ModernBERT + Ettin + GTE already cover the long-context cell. Re-evaluate if a nomic-specific finding arises.
